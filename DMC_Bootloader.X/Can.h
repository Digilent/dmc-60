...
/* Define the maximum number of received CAN messages that can be processeed
** during a single call to CanInterruptHandler().
*/
#define ccmsgProcMax            16
...
/* Define the index of the TX status frames within the CAN message buffer.
*/
...
void    CanInit();
void    CanTerm();
BOOL    CanIsLinkPresent();

void    CanTick();
void    CanAckEnterBootloader();
...
void    CanCmdEraseWriteFlashPage(VENDORCMD* pvcmd);
